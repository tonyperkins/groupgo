...
async def admin_new_poll(request: Request, db: Session = Depends(get_db)):
    """Redirect to the new React SPA creation wizard."""
    return RedirectResponse(url="/vote/admin/new", status_code=302)


@router.get("/polls/wizard", response_class=HTMLResponse)
async def admin_creation_wizard(request: Request, db: Session = Depends(get_db)):
    """Legacy route redirecting to the new React SPA wizard."""
    return RedirectResponse(url="/vote/admin/new", status_code=302)
